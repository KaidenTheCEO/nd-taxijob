...
RegisterNetEvent('NDCore:characterLoaded', function()
    PlayerJob = NDCore:getPlayer().job
    if Config.UseTarget then
        setupTarget()
        setupCabParkingLocation()
                ...
                if dist < 25 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            lib.notify({
                                title = Lang:t('info.person_was_dropped_off'),
                                description = '',
                                type = 'success',
                                position = 'top-right'
                            })
            ...
            if NpcData.NpcTaken then
                ...
                if vehicle and vehicle ~= 0 then
                    ...
                    if currentHealth < Config.Advanced.MinCabHealth then
                        ...
                        return lib.notify({
                            title = Lang:t('error.broken_taxi'),
                            description = '',
                            type = 'error',
                            position = 'top-right'
                        })
                    ...
                    if lastVehicleHealth and currentHealth < lastVehicleHealth then
                        ...
                        if NpcData.CrashCount >= Config.Advanced.MaxCrashesAllowed then
                            TriggerEvent('qb-taxi:client:CancelTaxiNpc')
                            return lib.notify({
                                title = Lang:t('error.ride_canceled'),
                                description = '',
                                type = 'error',
                                position = 'top-right'
                            })
                        ...
                        lib.notify({
                            title = string.format(Lang:t('error.crash_warning'), count, count == 1 and Lang:t('error.time') or Lang:t('error.times')),
                            description = '',
                            type = 'error',
                            position = 'top-right'
                        })
...
-- Register the context menu for the taxi garage
local function registerTaxiGarageMenu()
    -- Define the context menu
    local vehicleMenu = {
        id = 'vehicleMenu', -- Unique ID for the context menu
        title = Lang:t('menu.taxi_menu_header'), -- Title of the menu
        canClose = true, -- Allow the menu to be closed
        options = {}
    }

    -- Populate the menu with vehicle options
    for _, v in pairs(Config.AllowedVehicles) do
        table.insert(vehicleMenu.options, {
            title = v.label,
            event = 'qb-taxi:client:TakeVehicle', -- Event to trigger when this option is selected
            args = { model = v.model } -- Arguments to pass to the event
        })
    end

    -- Add the close menu option
    table.insert(vehicleMenu.options, {
        title = Lang:t('menu.close_menu'),
        event = 'lib.hideContext' -- Event to hide the context menu
    })
...
-- Call the function to register the menu
registerTaxiGarageMenu()

-- Function to show the taxi garage menu
function TaxiGarage()
    lib.showContext('vehicleMenu') -- Show the registered context menu
end
...
RegisterNetEvent('qb-taxi:client:TakeVehicle', function(data)
    ...
    if SpawnPoint then
        ...
        if CanSpawn then
            ...
            local veh = CreateVehicle(model, coords, Config.CabSpawns[SpawnPoint].w, true, false)
            SetVehicleNumberPlateText(veh, 'TAXI' .. tostring(math.random(1000, 9999)))
            exports['LegacyFuel']:SetFuel(veh, 100.0)
            lib.hideContext() -- updated to use ox-lib
            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
            SetVehicleEngineOn(veh, true, true)
            -- {{ edit_1 }}
        else
            lib.notify({
                title = Lang:t('info.no_spawn_point'),
                description = '',
                type = 'error',
                position = 'top-right'
            })
...
RegisterNetEvent('qb-taxi:client:DoTaxiNpc', function()
    -- if not PlayerJob.onduty then return end -- duty check removed
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            lib.notify({
                title = Lang:t('error.already_mission'),
                description = '',
                type = 'error',
                position = 'top-right'
            })
        ...
        lib.notify({
            title = Lang:t('error.not_in_taxi'),
            description = '',
            type = 'error',
            position = 'top-right'
        })
...
RegisterNetEvent('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            lib.notify({
                title = Lang:t('error.missing_meter'),
                description = '',
                type = 'error',
                position = 'top-right'
            })
...
RegisterNetEvent('qb-taxi:client:enableMeter', function()
    if meterIsOpen then
        ...
        lib.notify({
            title = Lang:t('error.not_active_meter'),
            description = '',
            type = 'error',
            position = 'top-right'
        })
...
CreateThread(function()
    while true do
        if not Config.UseTarget then
            local inRange = false
            local Player = NDCore:getPlayer(src) -- Attempt to get the player data
            if Player then -- Check if Player is valid
                ...
                -- If Player is nil, wait and reattempt
                Wait(1000) -- Wait before reattempting to avoid tight loops
            end
            if not inRange then
                Wait(3000)
            end
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                -- {{ edit_1 }}
                lib.showTextUI(Lang:t('info.call_npc'), { position = 'right-center' }) -- updated to use ox-lib
                callNpcPoly()
                -- {{ edit_1 }}
            end
        else
    ...
    deliveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                -- {{ edit_1 }}
                lib.showTextUI(Lang:t('info.drop_off_npc'), { position = 'right-center' }) -- updated to use ox-lib
                dropNpcPoly()
                -- {{ edit_1 }}
            end
        else
    ...
    CreateThread(function()
        while not NpcData.NpcTaken do
            ...
            if isInsidePickupZone then
                if IsControlJustPressed(0, 38) then
                    -- {{ edit_1 }}
                    lib.hideTextUI() -- updated to use ox-lib
                    -- {{ edit_1 }}
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                lib.showTextUI(Lang:t('info.vehicle_parking'), { position = 'right-center' })
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    lib.notify({
                        title = Lang:t('info.taxi_returned'),
                        description = '',
                        type = 'success',
                        position = 'top-right'
                    })
